...
int main(void)
{
	...
	while (true) {
		...
		for (int i = 1;i < cnt;i++) {
			...
			if (i == 1) { //최소값 구하기(c[1]) 초기화
				c1[1] = abs(sqrt(pow(l_pt.x - pp.x, 2) + pow(l_pt.y - pp.y, 2)));
				c2[1] = abs(sqrt(pow(l_pt.x - pp.x, 2) + pow(l_pt.y - pp.y, 2)));
			}
		...
		if (n1 > 0 ) { //왼쪽라인 바운딩 박스
			rectangle(dst, Rect(stats.ptr<int>(n1)[0], stats.ptr<int>(n1)[1], stats.ptr<int>(n1)[2], stats.ptr<int>(n1)[3]), Scalar(0, 255, 0), 3);
		}
		if (n2 > 0) {  //오른쪽라인 바운딩 박
			rectangle(dst, Rect(stats.ptr<int>(n2)[0], stats.ptr<int>(n2)[1], stats.ptr<int>(n2)[2], stats.ptr<int>(n2)[3]), Scalar(0, 255, 0), 3);
		}
